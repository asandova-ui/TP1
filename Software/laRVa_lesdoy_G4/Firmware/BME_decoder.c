...
{ 
	...
	SPISS0  = 0b01;          // BME_ADC = 0 
	SPIDAT0 = (1<<7) | addr; // RD (bit 7) = 1 y dirección (bits 6..0) 
	while(SPISTA0 & 1);      // Esperamos a que se complete la transferencia 
...
uint32_t BME_gettemperature(int return_tfine)
{
...
uint32_t BME_getpressure(){
	...
	uint32_t var1=(BME_gettemperature(1)>>1)-64000;
...
uint32_t BME_gethumidity(){
	uint32_t temp_scaled=BME_gettemperature(0);
	union partX par_h1;
	union partX par_h2;
	union partX par_h3;
	union partX par_h4;
	...
	uint32_t var1=hum_adc.combined - (par_h1<<4) - (temp_scaled * par_h3.combined /((uint32_t)100) >>1);
...
uint8_t BME_heat_gas(uint32_t target_temp)
{
	...
	res_heat_range.parts.bytelow=(BME_read(0x02)<<2)>>6;
	res_heat_val.parts.bytelow=0x00;
	
	uint32_t var1= (BME_gettemperature(0) * par_g3.combined /10)<<8;
	uint32_t var2 = (par_g1.combined + (uint32_t)784) * (((((par_g2.combined + 154009) * target_temp * 5)/100)+3276800)/10);
